  ...
  useEffect(() => {
    if (client) {
      setForm({
        ...
        pricing_type: client.pricing_type ?? "retail",
        discount_default: client.discount_default?.toString() ?? "",
        credit_limit: client.credit_limit?.toString() ?? "",
        payment_terms: client.payment_terms ?? "prepay",
        commission_rate: client.commission_rate?.toString() ?? "",
