...
> Живой документ. Описывает фактически реализованную систему по состоянию на **2026-05-21**.
> При добавлении новых модулей/таблиц/правил — обновляйте соответствующий раздел и дату в шапке.
>
> **Изменение 2026-05-21:** Срок изготовления во всех КП и шаблонах указывается
> в **календарных днях** (ранее — рабочих). Подпись настройки `production_days`
> в `pricing_settings` обновлена соответственно.
...
8 секций верхнего меню:
1. **Калькуляторы** — Цены, Цвета, Иконки продуктов, Расчёты.
2. **Справочники** — Номенклатура, Категории товаров, Справочники системы (`dictionary_items` с CRUD и `semantic_tags`).
3. **Продажи** — Контрагенты (управление клиентами в CRM).
4. **Производство** — этапы управляются через справочники (`production_stage`, `production_order_status`).
5. **Склад** — описание параметров резервирования.
6. **Интеграции** — очередь обмена с 1С, WordPress webhook.
7. **Системные настройки** (отдельный пункт) — полный CRUD над таблицей `system_settings`:
   НДС, гарантия, часы резерва (paid/unpaid), порог согласования скидки, любые пользовательские ключи
   (number / string / boolean / json), сгруппированные по категориям.
8. **Система** — Пользователи, Группы доступа, Контакты компании, Шаблон печати.

Все справочники и системные настройки поддерживают создание, редактирование, отключение
(`is_active=false`) и удаление. Бизнес-логика опирается на `semantic_tags`, а не на конкретные
коды, поэтому переименование/добавление статусов не ломает приложение.
